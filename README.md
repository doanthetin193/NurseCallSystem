...
> **Đồ án môn Lập Trình Mạng** — Ứng dụng mạng LAN mô phỏng hệ thống gọi y tá tại bệnh viện, tích hợp đầy đủ 8 chương kiến thức lập trình mạng.
...
## 📚 Kiến thức mạng sử dụng (8 Chương)

| Chương | Chủ đề | Hiện thực trong project |
|--------|--------|------------------------|
| **1** | Giới thiệu lập trình mạng | Mô hình Client–Server, địa chỉ IP, Port, giao thức TCP/UDP |
| **2** | Socket hướng kết nối (TCP) | `TcpListener` / `TcpClient` nhận 3 loại cảnh báo + gói OFFLINE, phản hồi `ACK_ALERT` |
| **3** | Socket không hướng kết nối (UDP) | Broadcast `NURSE_CALL_DISCOVERY` tự động tìm Server; `GET_ACTIVE_ROOMS` tải danh sách phòng |
| **4** | Lớp Helper C# | `StreamReader` / `StreamWriter` / `NetworkStream` bọc TCP; `NetworkCrypto` mã hóa XOR toàn bộ giao tiếp |
| **5** | Lập trình bất đồng bộ | `async/await` xuyên suốt: `AcceptTcpClientAsync`, `ReceiveAsync`, `SendPingAsync` — UI không bao giờ đứng hình |
| **6** | Thread trong ứng dụng mạng | `Task.Run()` chạy các vòng lặp mạng độc lập; `InvokeRequired` + `Invoke` đồng bộ ngược lại UI thread |
| **7** | IP Multicasting | Nút "Báo Động Toàn Viện" gửi `CODE_BLUE_EVACUATE` đến địa chỉ `239.0.0.1:50002`; Server replay Code Blue cho client mới kết nối; gửi `RESOLVED` qua cùng kênh multicast |
| **8** | ICMP và ứng dụng | `IcmpHealthCheckLoopAsync` ping từng IP mỗi 5 giây — phát hiện thiết bị mất mạng, tự chuyển Offline (không ghi đè cảnh báo đang active) |

### Bảo mật (Yêu cầu bổ sung)
- **Mã hóa XOR** (`NetworkCrypto`): Toàn bộ gói tin UDP, TCP, Multicast đều được mã hóa trước khi truyền
- TCP dùng **Base64 + XOR**: Đảm bảo byte rác sau XOR không phá vỡ định dạng text của `StreamReader`
...
│   │   └── NetworkCrypto.cs          # Mã hóa/giải mã XOR
│   │
│   ├── Data/
...
└── PatientClient/                # Project: Thiết bị đầu giường bệnh nhân
    ...
    └── Net/
        ...
        └── NetworkCrypto.cs          # Mã hóa/giải mã XOR (bản Client)
```
...
| Port | Giao thức | Hướng | Nội dung (sau giải mã XOR) |
|------|-----------|-------|---------------------------|
...
| 50001 | TCP | Client → Server | `ALERT\|{type}\|{room}\|{bed}` (Base64+XOR) |
| 50001 | TCP | Server → Client | `ACK_ALERT` (Base64+XOR) |
...
*Đồ án môn Lập Trình Mạng — .NET Framework 4.7.2 — Windows Forms*
